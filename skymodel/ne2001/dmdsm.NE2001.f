...
c 1992-1993: TC93 version
      ...
      subroutine dmdsm(
     .     datadir,l,b,ndir,dmpsr,dist,limit,sm,smtau,smtheta,smiso)
...
Cf2py intent(in) datadir      
Cf2py intent(in) l
Cf2py intent(in) b
Cf2py intent(in) ndir
Cf2py intent(in,out) dmpsr
Cf2py intent(in,out) dist
Cf2py intent(out) limit
Cf2py intent(out) sm
Cf2py intent(out) smtau
Cf2py intent(out) smtheta
Cf2py intent(out) smiso
...
c through read-in of parameter file:
        ...
        y = R0
        z = 0.0
            call density_2001(datadir,x,y,z,
...
c  routine will work for n_e models with large n_e near the Sun.
	...
	do 10 i=1,99999
     ...
     .        hitclump, hitvoid, wvoid)
	  ...
	  if(ndir.ge.3) then 
            call density_2001(datadir,x+dx0,y+dy0,z+dz0,
     .        ne1,ne2,nea,negc,nelism,necN,nevN,
     .        F1val, F2val, Faval, Fgc, Flism, FcN, FvN,
     .        whicharm, wlism, wldr, wlhb, wlsb, wloopI,
     .        hitclump, hitvoid, wvoid)
