...
c	the clump or void component is flagged out.
      ...
      SUBROUTINE DENSITY_2001(datadir,x,y,z,
...
c need to initiate values in case components are flagged out
	...
	if(wga .eq. 1) nea = ne_arms_log_mod(datadir, x,y,z,whicharm,Fa)
        if(wggc .eq. 1) negc = ne_gc(datadir,x,y,z, Fgc)
        if(wglism .eq. 1) 
     .       nelism = ne_lism(datadir,
     .   x,y,z,Flism,wlism,wldr,wlhb,wlsb,wloopI)
        if(wgcN .eq. 1) call neclumpN(datadir,x,y,z,necN,FcN,hitclump)
        if(wgvN .eq. 1) call nevoidN(datadir,
     .   x,y,z,nevN,FvN,hitvoid,wvoid)
...
c    .       whicharm
	...
	SUBROUTINE GET_PARAMETERS(datadir)
c-----------------------------------------------------------------------
	implicit none
        character(len=255) :: datadir
	real rsun
...
c	farm:	factors that multiply n_e^2 when calculating SM
	...
	real negc0, Fgc0
	   ...
	   open(11,file=trim(datadir) // '/gal01.inp',status='old')
	   read(11,*)
...
c    .           farm(1), farm(2), farm(3), farm(4), farm(5) 
	...
	REAL FUNCTION NE_ARMS_LOG_MOD(datadir, x,y,z, whicharm, Farms)
c-----------------------------------------------------------------------
...
c (referred to as 'Galactic center component' in circa TC93 density.f)
	...
	return
      ...
      REAL FUNCTION NE_GC(datadir, x, y, z, F_gc)
c-----------------------------------------------------------------------
...
c     parameter (hgc=0.026)
     ...
     .                na,ha,wa,Aa,Fa 
      ...
      if(first) then
         open(11, file=trim(datadir) // '/ne_gc.inp', status='old')
          read(11,*)
          read(11,*) xgc, ygc, zgc
	  read(11,*) rgc
	  read(11,*) hgc
