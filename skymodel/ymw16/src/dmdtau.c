 ...
 //The localtion of Sun relative to GP and Warp
  ...
  for(i=1;i<=nk;i++){
	...
	if(i==nk){ 
	  ...
	  if(np==0 && vbs>=1)printf(" Dist:%9.1f log(tau_sc):%7.3f %s\n",dist, log10(tau_MC_sc),text);
	  *dmord=dist;
	  if(np==1 && vbs>=1)printf(" DM_Gal:%8.2f Dist:%9.1f log(tau_sc):%7.3f %s\n", dmm, dist,log10(tau_sc),text);
	}	    
        if(vbs>=2){
	  printf("dmm=%lf\n", dmm);
	}
	...
	if(np!=1&&umc==1){
	  if(R_g>35000){ 
        ...
        if(i==nk&&np!=-1){
          ...
          tau_sc=tsc(dmpsr);
          tau_MC_sc=MAX(tau_Gal, tau_MC);
          if(np==0 && vbs>=1)printf(" DM:%8.2f log(tau_sc):%7.3f %s\n", dmpsr,log10(tau_MC_sc),text);
          if(np==1 && vbs>=1)printf(" DM:%8.2f log(tau_sc):%7.3f %s\n", dmpsr, log10(tau_sc),text);
        }
